...
@section('content')
    <div class="space-y-8">
        ...
        <div class="glass bg-white dark:bg-darkbg/40 p-4 md:p-6 rounded-2xl border border-slate-200/10 dark:border-white/5 shadow-sm">
            <form action="{{ route('patients.index') }}" method="GET" class="flex flex-col md:flex-row items-center gap-4">
                ...
                <div class="flex items-center space-x-3 w-full md:w-auto">
                    ...
                    @if(request()->filled('search'))
                        <a href="{{ route('patients.index') }}" class="px-6 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-danger rounded-2xl transition-all text-[10px] font-black uppercase tracking-widest border border-transparent hover:border-danger/20">
                            Reset
                        </a>
                    @endif
                </div>
                                    ...
                                    <td class="p-6">
                                        <div class="flex items-center space-x-3">
                                            <div class="w-8 h-8 rounded-lg overflow-hidden ring-2 ring-slate-100 dark:ring-slate-800">
                                                @if($patient->creator->profile && $patient->creator->profile->profile_picture)
                                                    <img src="{{ asset('storage/' . $patient->creator->profile->profile_picture) }}" class="w-full h-full object-cover">
                                                @else
                                                    <div class="w-full h-full bg-slate-100 dark:bg-slate-800 text-slate-400 flex items-center justify-center text-[10px] font-black">
                                                        {{ substr($patient->creator->profile->full_name ?? 'U', 0, 1) }}
                                                    </div>
                                                @endif
                                            </div>
                                            <div>
                                                <p class="text-xs font-bold text-slate-700 dark:text-slate-200">{{ $patient->creator->profile->full_name ?? 'Unknown' }}</p>
