...
@section('title', 'Patient BIN Recovery')
@section('header_title', 'Patient Recovery Bin')
        ...
        <div class="p-8 border-b border-slate-50 dark:border-white/5 flex items-center justify-between">
            <div>
                <h3 class="font-black text-xl text-slate-800 dark:text-white uppercase tracking-tight">Recently Deleted
                    Patients</h3>
                <p class="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Found {{ $patients->total() }}
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-white/2 transition-colors">
                            <td class="px-8 py-6">
                                <div class="flex items-center space-x-4">
                                    ...
                                    <div>
                                        <p class="text-sm font-black text-slate-800 dark:text-white">{{ $patient->full_name }}
                                        </p>
                            ...
                            <td class="px-8 py-6">
                                ...
                                <p class="text-[10px] text-slate-400 uppercase font-black tracking-tighter">
                                    {{ $patient->creator->employee_id ?? 'N/A' }}</p>
                            </td>
                            <td class="px-8 py-6">
                                <p class="text-sm font-black text-slate-700 dark:text-slate-200">
                                    {{ $patient->deleted_at->format('d M, Y') }}</p>
                                <p class="text-[10px] text-slate-400 font-bold uppercase">
                                    {{ $patient->deleted_at->format('h:i A') }}</p>
                            </td>
                            <td class="px-8 py-6">
                                <div class="w-full max-w-[120px]">
                                    <div
                                        class="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-1.5">
                                        <div class="h-full {{ $daysLeft < 7 ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.4)]' : ($daysLeft < 15 ? 'bg-amber-500' : 'bg-emerald-500') }} transition-all"
                                            style="width: {{ $percentage }}%"></div>
                                    </div>
                                    <p
                                        class="text-[10px] font-black {{ $daysLeft < 7 ? 'text-rose-500' : 'text-slate-500' }} uppercase tracking-widest">
                                        {{ $daysLeft }} Days Remaining</p>
                                </div>
                            </td>
                                ...
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    @if(auth()->user()->isSuperAdmin())
                                        <form action="{{ route('patients.force-delete', $patient->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="button"
                                                onclick="confirmPermanentDelete({{ $patient->id }}, '{{ $patient->full_name }}')"
