        ...
        <div
            ...
            <form action="{{ route('surveys.update', $survey->id) }}" method="POST" class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="gender" required
                            ...
                            <option value="male" {{ old('gender', $survey->gender) == 'male' ? 'selected' : '' }}>Male
                            </option>
                            <option value="female" {{ old('gender', $survey->gender) == 'female' ? 'selected' : '' }}>Female
                            </option>
                            <option value="other" {{ old('gender', $survey->gender) == 'other' ? 'selected' : '' }}>Other
                            </option>
                        </select>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-black text-slate-500 uppercase tracking-widest">Phone
                            Number</label>
                        <input type="tel" name="phone_number" value="{{ old('phone_number', $survey->phone_number) }}"
                ...
                @php
                    ...
                    $currentHealth = old('health_issues', $survey->health_issues);
                ...
                <div class="space-y-4">
                    ...
                    <div id="health-other-container"
                        ...
                        <textarea id="health-other-input" name="health_issue_other" rows="3"
                            class="w-full px-5 py-4 bg-slate-100/50 dark:bg-slate-800 border-2 border-transparent focus:border-accent focus:bg-white dark:focus:bg-slate-700 rounded-2xl transition-all outline-none text-sm font-bold text-slate-700 dark:text-white resize-none"
                            placeholder="Enter the specific health issue here...">{{ old('health_issue_other', $otherContent) }}</textarea>
                    </div>
                </div>
