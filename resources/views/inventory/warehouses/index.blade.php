    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                            @forelse($warehouses as $warehouse)
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                                    <td class="p-4">
                                        <span class="font-bold text-sm">{{ $warehouse->name }}</span>
                                    </td>
                                    <td class="p-4">
                                        <span
                                            class="text-xs text-slate-500">{{ $warehouse->location ?? 'N/A' }}</span>
                                    </td>
                                    <td class="p-4">
                                    ...
                                    <td class="p-4 text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            <button
                                                onclick="editWarehouse({{ $warehouse->id }}, '{{ $warehouse->name }}', '{{ $warehouse->location }}', {{ $warehouse->is_active ? 1 : 0 }})"
                                            ...
                                            <form action="{{ route('inventory.warehouses.destroy', $warehouse->id) }}"
        ...
        <div class="md:col-span-1">
            <div
                ...
                <form id="warehouse-form" action="{{ route('inventory.warehouses.store') }}" method="POST">
                    @csrf
...
@section('js')
    <script>
        ...
        function resetForm() {
            ...
            form.action = "{{ route('inventory.warehouses.store') }}";
