...
@section('content')
    <div
        ...
        <div class="p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
            <div>
                <h3 class="font-bold text-lg">Inventory Movements</h3>
                <p class="text-sm text-slate-500">A detailed chronological log of all stock changes.</p>
            </div>
            <a href="{{ route('inventory.index') }}"
                class="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-bold hover:bg-slate-200 transition">
                Back to Overview
            </a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-white/5">
                        ...
                        <th class="p-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Medicine & Location
                        </th>
                        ...
                        <th class="p-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Performed By</th>
                        <th class="p-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Entity Link</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                            ...
                            <td class="p-4">
                                <div class="flex flex-col">
                                    <span
                                        class="font-bold text-sm">{{ $transaction->stock?->medicine?->name ?? 'Deleted Medicine' }}</span>
                                    ...
                                    @if($transaction->sponsor)
                                        <div class="mt-1">
                                            <span
                                                class="px-1.5 py-0.5 bg-blue-50 dark:bg-blue-500/10 text-blue-500 text-[8px] font-black rounded uppercase border border-blue-500/20">
                                                Sponsor: {{ $transaction->sponsor->name }}
                                            </span>
                                        </div>
                                    @endif
                                    @if($transaction->notes)
                                        <p class="text-[9px] text-slate-400 mt-1 italic">{{ Str::limit($transaction->notes, 30) }}
                                        </p>
                                    @endif
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-10 text-center text-slate-500 font-medium">No transactions recorded yet.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div
                ...
                <form id="edit-form" method="POST">
                    ...
                    <div class="bg-white dark:bg-slate-800 px-8 pt-8 pb-4">
                        <h3 class="text-lg font-black text-slate-900 dark:text-white mb-6" id="modal-title">Edit Transaction
                        </h3>
