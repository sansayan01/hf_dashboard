...
@section('header_title', 'NGO Pharmacy | Dispensing')
            ...
            <div
                ...
                @if($patient)
                    <div
                        ...
                        Selected Patient: {{ $patient->full_name }}
                    </div>
                @endif
            </div>

            <form action="{{ route('inventory.process-dispense') }}" method="POST" class="p-8 space-y-8">
                @csrf
                                ...
                                <select name="patient_id" required
                                    ...
                                    @foreach($patients as $p)
                                        <option value="{{ $p->id }}" {{ ($patient && $patient->id == $p->id) ? 'selected' : '' }}>
                                            {{ $p->full_name }} (#{{ $p->id }}) - {{ $p->phone_number }}
                                        </option>
                                    @endforeach
                                </select>
