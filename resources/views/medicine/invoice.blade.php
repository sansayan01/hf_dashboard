...
@section('title', 'Invoice #' . $distribution->id)
        ...
        <div id="invoice-card" class="bg-white py-12 px-10 md:px-16 rounded-3xl shadow-xl border border-slate-100 print:shadow-none print:border-0 print:p-0">
            ...
            <div class="flex justify-between items-start mb-12 border-b border-slate-100 pb-8">
                ...
                <div class="text-right">
                    ...
                    <div class="text-xl font-bold text-slate-800">
                        #INV-{{ str_pad($distribution->id, 6, '0', STR_PAD_LEFT) }}</div>
                    <div class="text-xs font-medium text-slate-500 mt-1">
                        {{ $distribution->created_at->format('M d, Y h:i A') }}</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-12 mb-12">
                <div>
                    ...
                    <h3 class="text-lg font-bold text-slate-800">{{ $distribution->patient->full_name }}</h3>
                    <p class="text-sm text-slate-500 mt-1">{{ $distribution->patient->patient_id }}</p>
                    <p class="text-sm text-slate-500 mt-1">{{ $distribution->patient->address }}</p>
                    @if($distribution->patient->phone_number)
                        <p class="text-sm text-slate-500 mt-1">{{ $distribution->patient->phone_number }}</p>
                    @endif
                </div>
                <div class="text-right">
                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4">Dispensed From</p>
                    <h3 class="text-lg font-bold text-slate-800">{{ $distribution->camp->name }}</h3>
                    <p class="text-sm text-slate-500 mt-1">{{ $distribution->camp->location }}</p>
                    <div class="mt-4">
                        <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Pharmacist</p>
                        <p class="text-sm font-bold text-slate-700">
                            {{ $distribution->pharmacist->profile->full_name ?? $distribution->pharmacist->employee_id }}
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <tr>
                            <td class="py-4 font-bold text-slate-700">
                                ...
                                @if($item->medicine->dosage)
                                    <span class="text-xs font-normal text-slate-400 ml-1">({{ $item->medicine->dosage }})</span>
                                @endif
                            </td>
                            ...
                            <td class="py-4 text-right font-bold text-slate-800">{{ number_format($item->total_price, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
                ...
                <div class="flex justify-between items-start">
                    ...
                    <div class="text-right space-y-2">
                        <div class="flex justify-end items-center space-x-8">
                            ...
                            <p class="text-sm font-bold text-slate-700">₹{{ number_format($distribution->total_amount, 2) }}</p>
                        </div>
                        <div class="flex justify-end items-center space-x-8">
                            <p class="text-[10px] font-black uppercase tracking-widest text-emerald-500">Discount ({{ $distribution->discount_percentage }}%)</p>
                            <p class="text-sm font-bold text-emerald-500">-₹{{ number_format($distribution->discount_amount, 2) }}</p>
                        </div>
                        <div class="pt-2">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Grand Total</p>
                            <p class="text-4xl font-black text-accent">₹{{ number_format($distribution->final_amount, 2) }}</p>
                        </div>
                    </div>
                </div>
            </div>
