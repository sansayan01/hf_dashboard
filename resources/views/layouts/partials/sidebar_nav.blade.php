...
<ul class="space-y-1">
    ...
    @if($effectiveUser->isSuperAdmin())
        <li>
            <div x-data="{ open: {{ request()->routeIs('camp_records.*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-4 py-3 rounded-lg text-bodydark hover:text-white hover:bg-secondary transition font-medium {{ request()->routeIs('camp_records.*') ? 'bg-accent text-white shadow-lg' : '' }}">
                ...
                <div x-show="open" x-collapse class="pl-11 pr-4 mt-1 space-y-1">
                    <a href="{{ route('camp_records.index') }}"
                        class="block px-3 py-2 rounded-lg text-sm transition {{ request()->routeIs('camp_records.*') ? 'text-white bg-white/10 font-bold' : 'text-slate-400 hover:text-white hover:bg-white/5' }}">
                        Camp Records
                    </a>
                </div>
            </div>
