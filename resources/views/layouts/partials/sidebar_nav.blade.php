    ...
    @if($effectiveUser->isSuperAdmin())
        <li>
            <a href="{{ route('finances.index') }}"
                class="flex items-center space-x-3 px-4 py-3 rounded-lg text-bodydark hover:text-white hover:bg-secondary transition font-medium {{ (request()->routeIs('finances.*') || request()->routeIs('camp_records.*')) ? 'bg-accent text-white shadow-lg' : '' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
