...
@section('header_title', 'Messenger')
        ...
        <div
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar" id="conversationList">
                @forelse($conversations as $conversation)
                    ...
                    @endphp
                    <div onclick="loadConversation({{ $conversation->id }}, this)"
                        ...
                        <div class="flex items-center space-x-3">
                            <div class="relative">
                                <div
                                    ...
                                    <img src="{{ $otherUser->profile ? $otherUser->profile->getProfilePictureUrl() : 'https://ui-avatars.com/api/?name=' . urlencode($otherUser->designation ?? 'User') . '&color=7F9CF5&background=EBF4FF' }}"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between items-baseline mb-1">
                                    <h4
                                        ...
                                        {{ $otherUser->profile->full_name ?? $otherUser->designation }}
                                    </h4>
                                ...
                                <p
                                    class="text-xs text-slate-500 dark:text-slate-400 truncate {{ $isUnread ? 'font-bold text-slate-800 dark:text-slate-200' : '' }}">
                                    {{ $lastMsg ? ($lastMsg->sender_id == auth()->id() ? 'You: ' : '') . $lastMsg->body : 'Start a conversation' }}
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function startNewChat() {
            ...
            Swal.fire({
                ...
                preConfirm: () => {
                    window.location.href = "{{ route('users.index') }}"; // Redirect to team list for easier selection
                }
            });
        }
        ...
        async function sendMessage(e) {
            ...
            try {
                const res = await fetch(`{{ url('/messenger') }}/${activeConversationId}/messages`, {
                    method: 'POST',
                    headers: {
