...
@section('header_title', 'Expense Tracker')
    ...
    <div class="max-w-7xl mx-auto pb-12">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-7">
            <div class="flex items-center gap-3">
                <a href="{{ route('finances.index') }}" class="text-slate-400 hover:text-indigo-600 transition">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('expenses.export', request()->query()) }}" class="btn-secondary">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            <div class="card p-5">
                ...
                <div class="text-[11px] text-slate-400 font-medium mt-1">{{ $totalCount }} entries</div>
            </div>
            ...
            <div class="card p-5">
                <div class="flex items-center justify-between mb-2">
                    ...
                    @if($monthChange != 0)
                        <span class="trend-badge {{ $monthChange > 0 ? 'trend-up' : 'trend-down' }}">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $monthChange > 0 ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"/>
                            </svg>
                            {{ abs($monthChange) }}%
                        </span>
                    @endif
                ...
                <div class="text-[11px] text-slate-400 font-medium mt-1">{{ now()->format('F Y') }}</div>
            </div>

            <div class="card p-5">
                <div class="stat-label mb-2">Avg. Daily Spend</div>
                <div class="stat-value">₹{{ number_format($avgDailySpend, 2) }}</div>
                <div class="text-[11px] text-slate-400 font-medium mt-1">Last 30 days</div>
            </div>

            <div class="card p-5">
                <div class="stat-label mb-2">Top Category</div>
                <div class="text-lg font-bold text-slate-900 dark:text-white truncate">{{ $topCategory->category ?? 'N/A' }}</div>
                <div class="text-[11px] text-slate-400 font-medium mt-1">{{ $topCategory ? '₹' . number_format($topCategory->total, 2) : '—' }}</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-5 mb-7">
            ...
            <div class="lg:col-span-2 chart-card">
                <div class="mb-5">
                    ...
                    <div class="chart-subtitle mt-0.5">{{ $categoryBreakdown->count() }} categories</div>
                </div>
                ...
                @if($categoryBreakdown->count() > 0)
                    <div class="mt-4 space-y-2 max-h-[110px] overflow-y-auto">
                        @php $pieColors = ['#4f46e5', '#7c3aed', '#a855f7', '#0891b2', '#059669', '#d97706', '#dc2626', '#db2777', '#64748b']; @endphp
                        @foreach($categoryBreakdown as $i => $cat)
                            <div class="flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full flex-shrink-0" style="background:{{ $pieColors[$i % count($pieColors)] }}"></span>
                                    <span class="font-semibold text-slate-600 dark:text-slate-300 truncate max-w-[120px]">{{ $cat->category }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-slate-800 dark:text-white" style="font-variant-numeric:tabular-nums">₹{{ number_format($cat->total, 2) }}</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-7">
            <div class="chart-card">
                ...
                @if($paymentBreakdown->count() > 0)
                    @php $maxPay = $paymentBreakdown->max('total'); @endphp
                    <div class="space-y-5">
                        @foreach($paymentBreakdown as $pm)
                            ...
                            <div>
                                <div class="flex justify-between mb-1.5">
                                    <span class="text-xs font-semibold text-slate-600 dark:text-slate-300">{{ \App\Models\Expense::PAYMENT_METHODS[$pm->payment_method] ?? $pm->payment_method }}</span>
                                    <span class="text-xs font-bold text-slate-800 dark:text-white" style="font-variant-numeric:tabular-nums">₹{{ number_format($pm->total, 2) }}</span>
                                </div>
                                <div class="pm-bar-track">
                                    <div class="pm-bar-fill" style="width:{{ $maxPay > 0 ? ($pm->total / $maxPay) * 100 : 0 }}%; background:{{ $bg }}"></div>
                                </div>
                                <div class="text-[10px] text-slate-400 font-medium mt-1">{{ $pm->count }} transactions</div>
                            </div>
                        @endforeach
                    </div>
                @else
                    ...
                    <div class="space-y-0.5">
                        @foreach($recentExpenses as $recent)
                            <div class="activity-item">
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-slate-800 dark:text-white truncate">{{ $recent->title }}</p>
                                    <p class="text-[11px] text-slate-400 font-medium">{{ $recent->category }} · {{ $recent->expense_date->format('d M') }}</p>
                                </div>
                                <span class="text-sm font-bold text-slate-900 dark:text-white ml-3 flex-shrink-0" style="font-variant-numeric:tabular-nums">₹{{ number_format($recent->amount, 2) }}</span>
                            </div>
                        @endforeach
                    </div>
                @else
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            <div class="card p-5">
                ...
                <div class="stat-value text-xl">₹{{ number_format($thisWeekTotal, 2) }}</div>
                <div class="flex items-center gap-1.5 mt-2">
                    @if($weekChange != 0)
                        <span class="trend-badge {{ $weekChange > 0 ? 'trend-up' : 'trend-down' }}" style="font-size:10px">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $weekChange > 0 ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"/>
                            </svg>
            ...
            <div class="card p-5">
                ...
                <div class="stat-value text-xl">₹{{ number_format($projectedMonthly, 2) }}</div>
                <div class="text-[11px] text-slate-400 font-medium mt-2">
                    Based on {{ now()->day }}/{{ now()->daysInMonth }} days pace
                </div>
            </div>

            <div class="card p-5">
                <div class="stat-label mb-2">Expense Frequency</div>
                <div class="stat-value text-xl">{{ $expensesPerWeek }}<span class="text-sm font-semibold text-slate-400">/wk</span></div>
            ...
            <div class="card p-5">
                ...
                <div class="flex items-end gap-2">
                    <div class="stat-value text-xl" style="color:{{ $burnColor }}">{{ $burnPercent }}%</div>
                ...
                <div class="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full mt-3 overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-700" style="width:{{ min($burnPercent, 100) }}%; background:{{ $burnColor }}"></div>
                </div>
            </div>
        </div>

        {{-- ── ADVANCED ANALYTICS ROW 4 ── --}}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-7">
            {{-- Day-of-Week Heatmap --}}
            <div class="chart-card">
            ...
            <div class="chart-card">
                ...
                @if($topExpenses->count() > 0)
                    @php $maxTop = $topExpenses->max('amount'); @endphp
                    <div class="space-y-3.5">
                        @foreach($topExpenses as $ri => $te)
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[140px]">{{ $te->title }}</span>
                                        <span class="text-xs font-bold text-slate-900 dark:text-white ml-2" style="font-variant-numeric:tabular-nums">₹{{ number_format($te->amount, 2) }}</span>
                                    </div>
                                    <div class="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full bg-indigo-500" style="width:{{ $maxTop > 0 ? ($te->amount/$maxTop)*100 : 0 }}%"></div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="px-5 pt-5 pb-4">
                    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            @endphp
                            @if($activeCount > 0)
                                <span class="px-2 py-0.5 text-[10px] font-bold bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-400 rounded-md">{{ $activeCount }} active</span>
                            @endif
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            @if($activeCount > 0)
                                <a href="{{ route('expenses.index') }}" class="text-xs font-semibold text-red-500 hover:text-red-700 transition">Clear All ×</a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        ...
                        @foreach($presets as $val => $label)
                            <button type="button" class="preset-pill {{ request('date_preset', '') == $val ? 'active' : '' }}"
                                onclick="setPreset('{{ $val }}')">
                                {{ $label }}
                            </button>
                        @endforeach
                        <input type="hidden" name="date_preset" id="datePresetInput" value="{{ request('date_preset', '') }}">
                    </div>

                    {{-- Search + Category + Payment Row --}}
                    <div class="flex flex-wrap items-end gap-3">
                        <div class="flex-1 min-w-[180px]">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search title, description, ref..."
                    ...
                    <div class="px-5 pb-5 pt-2 border-t border-slate-100 dark:border-white/5">
                        <div class="flex flex-wrap items-end gap-4">
                            <div class="min-w-[120px]">
                                ...
                                <input type="number" name="amount_min" value="{{ request('amount_min') }}" placeholder="0" step="0.01" min="0"
                                    class="filter-input" style="font-variant-numeric:tabular-nums">
                            </div>
                            <div class="min-w-[120px]">
                                <label class="filter-label">Max Amount (₹)</label>
                                <input type="number" name="amount_max" value="{{ request('amount_max') }}" placeholder="∞" step="0.01" min="0"
                                    class="filter-input" style="font-variant-numeric:tabular-nums">
                            </div>
                            <div class="min-w-[130px]">
                            ...
                            <div class="min-w-[130px]">
                                ...
                                <select name="sort_by" class="filter-input">
                                    <option value="expense_date" {{ request('sort_by','expense_date') == 'expense_date' ? 'selected' : '' }}>Date</option>
                ...
                @if($activeCount > 0)
                    <div class="px-5 py-3 border-t border-slate-100 dark:border-white/5 flex flex-wrap items-center gap-2">
                        ...
                        @if(request()->filled('search'))
                            <a href="{{ route('expenses.index', array_diff_key(request()->query(), ['search'=>''])) }}" class="active-filter-tag">
                                Search: "{{ Str::limit(request('search'), 15) }}" ×
                            </a>
                        @endif
                        @if(request()->filled('category'))
                            <a href="{{ route('expenses.index', array_diff_key(request()->query(), ['category'=>''])) }}" class="active-filter-tag">
                                {{ request('category') }} ×
                            </a>
                        @endif
                        @if(request()->filled('payment_method'))
                            <a href="{{ route('expenses.index', array_diff_key(request()->query(), ['payment_method'=>''])) }}" class="active-filter-tag">
                                {{ \App\Models\Expense::PAYMENT_METHODS[request('payment_method')] ?? request('payment_method') }} ×
                            </a>
                        @endif
                        @if(request()->filled('date_preset'))
                            <a href="{{ route('expenses.index', array_diff_key(request()->query(), ['date_preset'=>''])) }}" class="active-filter-tag">
                                {{ $presets[request('date_preset')] ?? request('date_preset') }} ×
                            </a>
                        @endif
                        @if(request()->filled('amount_min'))
                            <a href="{{ route('expenses.index', array_diff_key(request()->query(), ['amount_min'=>''])) }}" class="active-filter-tag">
                                Min: ₹{{ request('amount_min') }} ×
                            </a>
                        @endif
                        @if(request()->filled('amount_max'))
                            <a href="{{ route('expenses.index', array_diff_key(request()->query(), ['amount_max'=>''])) }}" class="active-filter-tag">
                                Max: ₹{{ request('amount_max') }} ×
                            </a>
                        @endif
                        <span class="ml-auto text-[10px] font-bold text-slate-400">{{ $expenses->total() }} results</span>
                    </div>
                @endif
            </form>
        </div>
                    ...
                    <tbody>
                        @forelse($expenses as $expense)
                            <tr>
                                ...
                                <td class="text-right">
                                    <span class="font-bold text-slate-900 dark:text-white" style="font-variant-numeric:tabular-nums">₹{{ number_format($expense->amount, 2) }}</span>
                                </td>
                                <td>
                                    <span class="text-xs font-semibold text-slate-600 dark:text-slate-300">{{ \App\Models\Expense::PAYMENT_METHODS[$expense->payment_method] ?? $expense->payment_method }}</span>
...
@section('js')
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (dowCtx) {
            const dowData = @json($dayOfWeekSpending);
        ...
        if (radarCtx) {
            const catTrend = @json($categoryMonthlyTrend);
            const allCats = @json($allCategories);
