    ...
    <div class="max-w-7xl mx-auto pb-12">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            <div class="card p-5">
                ...
                <div class="stat-value" id="totalExpensesStat">₹{{ number_format($totalExpenses, 2) }}</div>
            ...
            <div class="card p-5">
                ...
                <div class="stat-value" id="avgDailyStat">₹{{ number_format($avgDailySpend, 2) }}</div>
                    ...
                    @foreach($paymentBreakdown as $pay)
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between">
                                <span
                                    class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ \App\Models\Expense::PAYMENT_METHODS[$pay->payment_method] ?? $pay->payment_method }}</span>
                                <span class="text-xs font-black text-slate-900 dark:text-white"
                                    style="font-variant-numeric:tabular-nums">₹{{ number_format($pay->total, 2) }}</span>
                            </div>
                            <div class="payment-bar-bg">
                                <div class="payment-bar-fill {{ $pay->payment_method == 'Bank Transfer' ? 'bg-indigo-500' : ($pay->payment_method == 'Cash' ? 'bg-emerald-500' : 'bg-slate-400') }}"
                                ...
                                <div class="activity-item">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-[11px] text-slate-400 font-medium">{{ $recent->category }} ·
                                            {{ $recent->expense_date->format('d M') }}
                                        </p>
                                    </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            ...
            <div class="card p-5">
                ...
                <div class="stat-value text-xl" id="expensesPerWeekStat">{{ $expensesPerWeek }}<span
            ...
            <div class="card p-5">
                ...
                <div class="flex items-end gap-2">
                    <div class="stat-value text-xl" id="burnPercentStat" style="color:{{ $burnColor }}">{{ $burnPercent }}%
                    </div>
                ...
                <div class="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full mt-3 overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-700" id="burnProgressBar"
                        style="width:{{ min($burnPercent, 100) }}%; background:{{ $burnColor }}"></div>
                </div>
            </div>
        </div>
        ...
        <div class="filter-panel mb-5">
            <form method="GET" action="{{ route('expenses.index') }}" id="filterForm">
                ...
                <div class="px-5 pt-5 pb-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if($activeCount > 0)
                                <span id="activeCountBadge"
                                    class="px-2 py-0.5 text-[10px] font-bold bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-400 rounded-md">{{ $activeCount }}
                                    active</span>
                            @else
                    ...
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <div class="min-w-[150px]">
                            ...
                            <input type="text" name="expense_by" value="{{ request('expense_by') }}"
                    ...
                    <div class="px-5 pb-5 pt-2 border-t border-slate-100 dark:border-white/5">
                        <div class="flex flex-wrap items-end gap-4">
                            ...
                            <div class="min-w-[120px]">
                                ...
                                <input type="number" name="amount_max" value="{{ request('amount_max') }}" placeholder="∞"
                                    step="0.01" min="0" class="filter-input" style="font-variant-numeric:tabular-nums">
                            </div>
                            <div class="min-w-[130px]">
                ...
                <div id="activeFilters">
                    @include('expenses.partials.active_filters')
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        async function fetchResults() {
            ...
            try {
                ...
                // Update Export URL
                document.getElementById('exportBtn').href = `{{ route('expenses.export') }}?${params.toString()}`;
