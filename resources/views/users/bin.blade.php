...
@section('title', 'BIN Recovery')
@section('header_title', 'Data Recovery Bin')
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($deletedUsers as $u)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-xs text-slate-400 font-medium">{{ $u->deleted_at->format('d M, Y') }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <div class="w-full max-w-[100px] h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                    <div class="h-full bg-warning" style="width: {{ ($daysLeft / 30) * 100 }}%"></div>
                                </div>
                                <p class="text-[10px] font-bold text-slate-500 mt-1 uppercase">{{ $daysLeft }} Days Left</p>
                            </td>
