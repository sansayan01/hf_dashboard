    ...
    <div class="glass-card">
        <div class="text-center mb-16">
            ...
            <p class="text-slate-400 text-lg font-medium">Select the document type you wish to generate for <span
                    class="text-white">{{ count($users) }} selected members</span>.</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <!-- ID Card Option -->
            <form action="{{ route('users.print-all-id-cards') }}" method="POST">
                @csrf
            ...
            <!-- Offer Letter Option -->
            <form action="{{ route('users.bulk-offer-letters') }}" method="POST">
                @csrf
                @foreach($selected_users as $id)
                    <input type="hidden" name="selected_users[]" value="{{ $id }}">
                @endforeach
