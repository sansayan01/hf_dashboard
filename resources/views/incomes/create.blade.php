...
@section('header_title', 'Add Income')
...
<div class="max-w-2xl mx-auto pb-12">
    ...
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('incomes.index') }}" class="back-btn">
    ...
    <div class="form-card">
        <form action="{{ route('incomes.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Monthly Donation"
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" value="{{ old('amount') }}" required step="0.01" min="0.01" placeholder="0.00"
                            class="field-input @error('amount') has-error @enderror" style="font-variant-numeric:tabular-nums">
            ...
            <div class="form-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        @error('category')<p class="text-xs text-rose-500 font-semibold mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="field-label">Date <span class="req">*</span></label>
                        <input type="date" name="income_date" value="{{ old('income_date', date('Y-m-d')) }}" required max="{{ date('Y-m-d') }}"
                            class="field-input @error('income_date') has-error @enderror" style="font-variant-numeric:tabular-nums">
                        @error('income_date')<p class="text-xs text-rose-500 font-semibold mt-1">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="payment_method" required class="field-input @error('payment_method') has-error @enderror">
                            @foreach(\App\Models\Income::PAYMENT_METHODS as $key => $label)
                                <option value="{{ $key }}" {{ old('payment_method', 'cash') == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('payment_method')<p class="text-xs text-rose-500 font-semibold mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="field-label">Reference Number</label>
                        <input type="text" name="reference_number" value="{{ old('reference_number') }}" placeholder="UPI / Cheque / Transaction ref"
                            class="field-input">
                    </div>
                    <div>
                        <label class="field-label">Received By <span class="req">*</span></label>
                        <input type="text" name="received_by" value="{{ old('received_by') }}" required list="received-by-list"
                            placeholder="Type or select a name..."
                            class="field-input @error('received_by') has-error @enderror">
                        <datalist id="received-by-list">
                            @foreach(\App\Models\Income::RECEIVED_BY_OPTIONS as $option)
                                <option value="{{ $option }}">
                            @endforeach
                        </datalist>
                        @error('received_by')<p class="text-xs text-rose-500 font-semibold mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="field-label">Source</label>
                        <input type="text" name="source" value="{{ old('source') }}" placeholder="Who or where did the funds come from?"
                            class="field-input @error('source') has-error @enderror">
                        @error('source')<p class="text-xs text-rose-500 font-semibold mt-1">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <textarea name="description" rows="2" placeholder="Brief description of the income..."
                            class="field-input resize-none">{{ old('description') }}</textarea>
                    </div>
            ...
            <div class="form-footer">
                <a href="{{ route('incomes.index') }}" class="btn-cancel">Cancel</a>
