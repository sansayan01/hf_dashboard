@if($incomes->count() > 0)
    @foreach($incomes as $income)
        <tr>
            ...
            <td>
                <div class="min-w-0">
                    <p class="text-sm font-bold text-slate-800 dark:text-white truncate max-w-[200px]">{{ $income->title }}</p>
                    @if($income->source)
                        <p class="text-[11px] text-slate-400 truncate max-w-[200px]">From: {{ $income->source }}</p>
                    @endif
                </div>
            </td>
            <td>
                ...
                @endphp
                <span class="cat-badge {{ $catColors[$income->category] ?? 'bg-slate-50 text-slate-600 border-slate-200' }}">
                    {{ $income->category }}
                </span>
            </td>
            <td>
            ...
            <td>
                <span class="text-xs font-medium text-slate-600 dark:text-slate-300">
                    {{ \App\Models\Income::PAYMENT_METHODS[$income->payment_method] ?? $income->payment_method }}
                </span>
            </td>
            <td>
                <span class="text-xs font-medium text-slate-600 dark:text-slate-300">
