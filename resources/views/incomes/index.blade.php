...
@section('header_title', 'Income Tracker')
    ...
    <div class="max-w-7xl mx-auto pb-12">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-7">
            <div class="flex items-center gap-3">
                <a href="{{ route('finances.index') }}" class="text-slate-400 hover:text-emerald-600 transition">
            ...
            <div class="flex items-center gap-3">
                @if(auth()->user()->hasPermission('finances.export_income'))
                    <a href="{{ route('incomes.export', request()->query()) }}" class="btn-secondary" id="exportBtn">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            <div class="card p-5">
                ...
                <div class="stat-value" id="totalIncomeStat">₹{{ number_format($totalIncome, 2) }}</div>
                <div class="text-[11px] text-slate-400 font-medium mt-1" id="totalCountStat">{{ $totalCount }} entries</div>
            </div>
            <div class="card p-5">
                        ...
                        @if($monthChange != 0)
                            <span class="trend-badge {{ $monthChange > 0 ? 'trend-up' : 'trend-down' }}">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="{{ $monthChange > 0 ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}" />
                                </svg>
                                {{ abs($monthChange) }}%
                            </span>
                        @endif
                ...
                <div class="stat-value" id="thisMonthTotalStat">₹{{ number_format($thisMonthTotal, 2) }}</div>
                <div class="text-[11px] text-slate-400 font-medium mt-1">vs last month</div>
            </div>
            <div class="card p-5">
                <div class="stat-label mb-2">Average Daily</div>
                <div class="stat-value" id="avgDailyStat">₹{{ number_format($avgDailyIncome, 2) }}</div>
            ...
            <div class="lg:col-span-2 chart-card">
                ...
                @if($categoryBreakdown->count() > 0)
                    <div class="mt-4 space-y-2 max-h-[110px] overflow-y-auto">
                        @php $pieColors = ['#059669', '#0891b2', '#7c3aed', '#4f46e5', '#d97706', '#a855f7', '#dc2626', '#db2777', '#0ea5e9', '#64748b']; @endphp
                        @foreach($categoryBreakdown as $i => $cat)
                            <div class="flex items-center justify-between text-xs">
                                <div class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full flex-shrink-0"
                                        style="background:{{ $pieColors[$i % count($pieColors)] }}"></span>
                                    <span
                                        class="font-semibold text-slate-600 dark:text-slate-300 truncate max-w-[120px]">{{ $cat->category }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-slate-800 dark:text-white"
                                        style="font-variant-numeric:tabular-nums">₹{{ number_format($cat->total, 2) }}</span>
                    ...
                    @foreach($paymentBreakdown as $pay)
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between">
                                <span
                                    class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ \App\Models\Income::PAYMENT_METHODS[$pay->payment_method] ?? $pay->payment_method }}</span>
                                <span class="text-xs font-black text-slate-900 dark:text-white"
                                    style="font-variant-numeric:tabular-nums">₹{{ number_format($pay->total, 2) }}</span>
                            </div>
                            <div class="payment-bar-bg">
                                <div class="payment-bar-fill {{ $pay->payment_method == 'bank_transfer' ? 'bg-emerald-500' : ($pay->payment_method == 'cash' ? 'bg-teal-500' : 'bg-slate-400') }}"
                                    style="width:{{ $maxPay > 0 ? ($pay->total / $maxPay) * 100 : 0 }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div id="recentActivityList">
                    @if($recentIncomes->count() > 0)
                        <div class="space-y-0.5">
                            @foreach($recentIncomes as $recent)
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
            <div class="card p-5">
                ...
                <div class="stat-value text-xl" id="thisWeekTotalStat">₹{{ number_format($thisWeekTotal, 2) }}</div>
                <div class="flex items-center gap-1.5 mt-2" id="weekChangeContainer">
                    @if($weekChange != 0)
                        <span class="trend-badge {{ $weekChange > 0 ? 'trend-up' : 'trend-down' }}" style="font-size:10px">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="{{ $weekChange > 0 ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}" />
                            </svg>
            ...
            <div class="card p-5">
                ...
                <div class="stat-value text-xl" id="projectedMonthlyStat">₹{{ number_format($projectedMonthly, 2) }}</div>
                <div class="text-[11px] text-slate-400 font-medium mt-2">Based on {{ now()->day }}/{{ now()->daysInMonth }}
                    days pace</div>
            </div>
            <div class="card p-5">
                <div class="stat-label mb-2">Income Frequency</div>
                <div class="stat-value text-xl" id="incomesPerWeekStat">{{ $incomesPerWeek }}<span
            ...
            <div class="card p-5">
                ...
                <div class="w-full h-2 bg-slate-100 dark:bg-slate-700 rounded-full mt-3 overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-700" id="growthProgressBar"
                        style="width:{{ min($growthPercent, 100) }}%; background:{{ $growthColor }}"></div>
                </div>
            </div>
        </div>
            ...
            <div class="chart-card">
                ...
                <div id="topIncomesList">
                    @if($topIncomes->count() > 0)
                        @php $maxTop = $topIncomes->max('amount'); @endphp
                        <div class="space-y-3.5">
                            @foreach($topIncomes as $ri => $te)
                                <div class="flex items-center gap-3">
                                    <span
                                        class="w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-black {{ $ri === 0 ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400' }}">{{ $ri + 1 }}</span>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between mb-1">
                                            <span
                                                class="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[140px]">{{ $te->title }}</span>
                                            <span class="text-xs font-bold text-slate-900 dark:text-white ml-2"
                                                style="font-variant-numeric:tabular-nums">₹{{ number_format($te->amount, 2) }}</span>
                                        </div>
                                        <div class="w-full h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                            <div class="h-full rounded-full bg-emerald-500"
                                                style="width:{{ $maxTop > 0 ? ($te->amount / $maxTop) * 100 : 0 }}%"></div>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
        ...
        <div class="filter-panel mb-5">
            <form method="GET" action="{{ route('incomes.index') }}" id="filterForm">
                <div class="px-5 pt-5 pb-4">
                    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            @php $activeCount = collect(['search', 'category', 'payment_method', 'date_from', 'date_to', 'date_preset', 'amount_min', 'amount_max'])->filter(fn($k) => request()->filled($k))->count(); @endphp
                            @if($activeCount > 0)
                                <span id="activeCountBadge"
                                    class="px-2 py-0.5 text-[10px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400 rounded-md">{{ $activeCount }}
                                    active</span>
                            @else
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        @php $presets = ['' => 'All Time', 'today' => 'Today', '7d' => 'Last 7 Days', '30d' => 'Last 30 Days', '90d' => 'Last 90 Days', 'this_month' => 'This Month', 'last_month' => 'Last Month', 'this_year' => 'This Year']; @endphp
                        @foreach($presets as $val => $label)
                            <button type="button" class="preset-pill {{ request('date_preset', '') == $val ? 'active' : '' }}"
                                onclick="setPreset('{{ $val }}', event)">{{ $label }}</button>
                        @endforeach
                        <input type="hidden" name="date_preset" id="datePresetInput"
                            value="{{ request('date_preset', '') }}">
                    </div>
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <div class="min-w-[140px]">
                            ...
                            <select name="category" class="filter-input">
                                ...
                                @foreach(\App\Models\Income::CATEGORIES as $cat)
                        ...
                        <div class="min-w-[150px]">
                            ...
                            <input type="text" name="received_by" value="{{ request('received_by') }}"
                                list="received-by-filter-list" placeholder="Search name..." class="filter-input">
                            <datalist id="received-by-filter-list">
                                @foreach(\App\Models\Income::RECEIVED_BY_OPTIONS as $option)
                                    <option value="{{ $option }}">
                                @endforeach
                            </datalist>
                        </div>
                    ...
                    <div class="px-5 pb-5 pt-2 border-t border-slate-100 dark:border-white/5">
                        <div class="flex flex-wrap items-end gap-4">
                            <div class="min-w-[120px]"><label class="filter-label">Min Amount (₹)</label><input
                                    type="number" name="amount_min" value="{{ request('amount_min') }}" placeholder="0"
                            ...
                            <div class="min-w-[100px]"><label class="filter-label">Direction</label>
                                <select name="sort_dir" class="filter-input">
                                    <option value="desc" {{ request('sort_dir', 'desc') == 'desc' ? 'selected' : '' }}>Newest
                                        First</option>
                                    <option value="asc" {{ request('sort_dir') == 'asc' ? 'selected' : '' }}>Oldest First
                                    </option>
                                </select>
                            </div>
                        </div>
                ...
                <div id="activeFilters">@include('incomes.partials.active_filters')</div>
            </form>
        </div>
        ...
        <div class="card overflow-hidden relative" id="tableWrapper">
            ...
            <div class="overflow-x-auto">
                <table class="data-table">
                    ...
                    <tbody id="incomeTableBody">@include('incomes.partials.table')</tbody>
                </table>
            </div>
            <div id="paginationContainer">
                @if($incomes->hasPages())
                    <div class="px-5 py-4 border-t border-slate-100 dark:border-white/5">{{ $incomes->links() }}</div>
                @endif
            </div>
        </div>
    </div>
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (dowCtx) {
                const dowData = @json($dayOfWeekIncome);
        ...
        async function fetchResults() {
            ...
            try {
                ...
                document.getElementById('exportBtn').href = `{{ route('incomes.export') }}?${params}`;
