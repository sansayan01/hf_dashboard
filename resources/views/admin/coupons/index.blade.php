...
<div class="container-fluid py-6 animate-in fade-in duration-700">
    ...
    <div class="relative mb-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
            ...
            <div class="flex items-center gap-3 ml-14 md:ml-0">
                ...
                <a href="{{ route('coupons.create') }}" class="group px-6 py-3 bg-accent text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-xl shadow-accent/20 hover:shadow-accent/40 hover:-translate-y-0.5 transition-all flex items-center gap-2">
            ...
            <div class="relative bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 rounded-3xl p-6">
                <div class="flex items-center justify-between pointer-events-none">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter">{{ $coupons->total() }}</h3>
                    </div>
            ...
            <div class="relative bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 rounded-3xl p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter">{{ $stats['unused'] }}</h3>
                    </div>
                ...
                <div class="mt-4 w-full bg-slate-100 dark:bg-white/5 h-1.5 rounded-full overflow-hidden">
                    <div class="bg-emerald-500 h-full transition-all duration-1000" style="width: {{ $coupons->total() > 0 ? ($stats['unused'] / $coupons->total()) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
            ...
            <div class="relative bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 rounded-3xl p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter">{{ $stats['used'] }}</h3>
                    </div>
                ...
                <div class="mt-4 w-full bg-slate-100 dark:bg-white/5 h-1.5 rounded-full overflow-hidden">
                    <div class="bg-blue-500 h-full transition-all duration-1000" style="width: {{ $coupons->total() > 0 ? ($stats['used'] / $coupons->total()) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
            ...
            <div class="relative bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 rounded-3xl p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter">{{ $stats['expired'] }}</h3>
                    </div>
                ...
                <div class="mt-4 w-full bg-slate-100 dark:bg-white/5 h-1.5 rounded-full overflow-hidden">
                    <div class="bg-amber-500 h-full transition-all duration-1000" style="width: {{ $coupons->total() > 0 ? ($stats['expired'] / $coupons->total()) * 100 : 0 }}%"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 mb-10 shadow-2xl shadow-slate-200/50 dark:shadow-none">
        <form method="GET" action="{{ route('coupons.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            <div class="flex flex-col gap-2">
                ...
                <select name="status" class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 rounded-2xl px-5 py-3 text-sm font-bold text-slate-600 dark:text-slate-300 focus:ring-4 focus:ring-accent/10 focus:border-accent transition-all outline-none">
                    ...
                    <option value="unused" {{ request('status') == 'unused' ? 'selected' : '' }}>Available (Unused)</option>
            ...
            <div class="lg:col-span-2 flex flex-col gap-2">
                ...
                <div class="relative group">
                    ...
                    <input type="text" name="search" placeholder="Enter HF-CASH-XXXXX..." value="{{ request('search') }}" 
        ...
        <div class="relative bg-white/80 dark:bg-slate-900/85 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-slate-200/50 dark:shadow-none">
            @if($coupons->count() > 0)
                            ...
                            @foreach($coupons as $coupon)
                                <tr class="group/row hover:bg-accent/5 dark:hover:bg-accent/5 transition-colors">
                                    ...
                                    <td class="px-8 py-6 text-right">
                                        <span class="text-lg font-black text-slate-800 dark:text-white tracking-tighter">₹{{ number_format($coupon->original_amount) }}</span>
                                    </td>
                                    ...
                                    <td class="px-8 py-6">
                                        @if($coupon->usedBy)
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="flex flex-col">
                                                    <a href="{{ route('users.show', $coupon->usedBy->id) }}" class="text-xs font-black text-slate-700 dark:text-slate-200 hover:text-accent transition-colors">
                                    ...
                                    <td class="px-8 py-6 text-right">
                                        <div class="flex justify-end gap-2">
                                            @if(!$coupon->is_used)
                                                <form action="{{ route('coupons.destroy', $coupon->id) }}" method="POST" class="inline" onsubmit="return confirm('Archive this unused coupon?');">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="px-8 py-6 border-t border-slate-50 dark:border-white/5 bg-slate-50/30 dark:bg-white/5">
                    <div class="flex flex-col md:flex-row justify-between items-center gap-4 text-xs font-bold text-slate-400 uppercase tracking-widest">
                        <div>Showing {{ $coupons->firstItem() }} - {{ $coupons->lastItem() }} of {{ $coupons->total() }} vouchers</div>
            ...
            @else
                <div class="flex flex-col items-center justify-center py-20 px-8 text-center">
                    ...
                    <a href="{{ route('coupons.create') }}" class="mt-8 px-8 py-4 bg-accent text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-accent/20 hover:shadow-accent/40 hover:-translate-y-1 transition-all">
