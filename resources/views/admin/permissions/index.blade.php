...
@section('header_title', 'Bulk Permission Control')
...
<div class="p-6 max-w-7xl mx-auto overflow-y-auto h-full pb-20">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @foreach($designations as $key => $designation)
            <a href="{{ route('admin.permissions.show', $key) }}"
               class="group relative block p-[3px] bg-gradient-to-br {{ $designation['gradient'] }} rounded-[2rem] shadow-xl hover:shadow-2xl transition-all hover:scale-[1.02] hover:-translate-y-1 duration-500">
                <div class="bg-white dark:bg-darkcard rounded-[1.9rem] p-6 h-full relative overflow-hidden">
                    {{-- Shimmer --}}
                    <div class="absolute -top-10 -right-10 w-40 h-40 bg-{{ $designation['color'] }}-500/10 rounded-full blur-3xl group-hover:scale-150 transition-transform duration-1000"></div>

                    <div class="relative z-10 flex flex-col h-full">
                        {{-- Short Badge --}}
                        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br {{ $designation['gradient'] }} flex items-center justify-center mb-5 group-hover:rotate-6 transition-transform duration-500 shadow-lg">
                            <span class="text-white font-black text-lg tracking-tight">{{ $designation['short'] }}</span>
                        </div>

                        {{-- Label --}}
                        <h3 class="text-lg font-black text-slate-800 dark:text-white uppercase tracking-tight mb-1 leading-tight">{{ $designation['label'] }}</h3>

                        {{-- User Count --}}
                        <p class="text-xs text-slate-400 dark:text-slate-500 font-bold mb-5">
                            {{ $designation['user_count'] }} {{ Str::plural('user', $designation['user_count']) }}
                        </p>
                        ...
                        <div class="mt-auto flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/5">
                            <div class="flex flex-col">
                                ...
                                <span class="text-sm font-black text-{{ $designation['color'] }}-500">{{ $designation['enabled_count'] }} / {{ $designation['total_count'] }}</span>
                            </div>

                            <div class="flex items-center gap-2">
                                @if($designation['has_overrides'])
                                    <span class="w-2 h-2 rounded-full bg-blue-500 animate-pulse" title="Custom overrides active"></span>
                                @endif
                                <div class="w-8 h-8 rounded-full bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-400 group-hover:bg-{{ $designation['color'] }}-500 group-hover:text-white transition-all duration-500">
