...
@section('title', 'Role Permissions — ' . $meta['label'])
@section('header_title', $meta['label'] . ' Permissions')
    ...
    <div class="bg-white dark:bg-darkcard rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-white/5 p-6 mb-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-3">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br {{ $meta['gradient'] }} flex items-center justify-center shadow-lg">
                        <span class="text-white font-black text-xl tracking-tight">{{ $meta['short'] }}</span>
                    </div>
                    <div>
                        <h1 class="text-xl font-black text-slate-800 dark:text-white">{{ $meta['label'] }}</h1>
                        <p class="text-sm text-slate-500 dark:text-slate-400 font-semibold">
                            Bulk editing for <strong class="text-{{ $meta['color'] }}-500">{{ $userCount }}</strong> {{ Str::plural('user', $userCount) }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 flex-wrap ml-auto">
                {{-- Reset Defaults --}}
                <form method="POST" action="{{ route('admin.permissions.reset', $designation) }}"
                      onsubmit="return confirm('Reset all permissions for {{ $meta['label'] }} to system defaults? This will remove all role-level overrides.')">
                    @csrf
                    <button type="submit"
        ...
        <div class="mt-4 flex items-center gap-2 flex-wrap">
            ...
            <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider ml-1">
                {{ collect($rolePermissions)->filter(fn($v) => $v)->count() }} / {{ count($rolePermissions) }} ENABLED
            </span>
            ...
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 text-[10px] font-bold uppercase border border-amber-100 dark:border-amber-500/20 ml-auto">
                ...
                {{ $userCount }} {{ Str::plural('User', $userCount) }} Affected
            </span>
        </div>
    </div>

    {{-- Main Form --}}
    <form method="POST" action="{{ route('admin.permissions.update', $designation) }}" id="permissionsForm">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white dark:bg-darkcard rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-white/5 overflow-hidden permission-category hover:border-blue-200 dark:hover:border-blue-500/20 transition-colors"
                    ...
                    <div class="w-full flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition group"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-left">
                                <h3 class="font-black text-slate-800 dark:text-white text-sm leading-tight uppercase tracking-tight">{{ $category['label'] }}</h3>
                                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                                    <span id="count-{{ $catKey }}">{{ collect($category['permissions'])->filter(fn($label, $key) => $rolePermissions[$key] ?? false)->count() }}</span> / {{ count($category['permissions']) }} enabled
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 group/header">
                            ...
                            <svg class="w-5 h-5 text-slate-300 dark:text-slate-600 transition-transform duration-300 ml-1 group-hover/header:text-blue-400" id="chevron-{{ $catKey }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div id="body-{{ $catKey }}" class="hidden border-t border-slate-100 dark:border-white/5">
                        <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                            @foreach($category['permissions'] as $key => $label)
                                @php $isEnabled = $rolePermissions[$key] ?? false; @endphp
                                <label class="flex items-center justify-between gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/[0.02] transition cursor-pointer group/perm">
                                    <div class="flex items-center gap-3">
                                        <div class="w-2 h-2 rounded-full {{ $isEnabled ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600' }} transition perm-dot"
                                             data-key="{{ $key }}"></div>
                                        <span class="text-sm font-semibold text-slate-700 dark:text-slate-300 group-hover/perm:text-slate-900 dark:group-hover/perm:text-white">{{ $label }}</span>
                                    </div>
        ...
        <div class="sticky bottom-0 bg-white/80 dark:bg-darkcard/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 -mx-4 sm:-mx-6 px-4 sm:px-6 py-4 mt-6 rounded-b-2xl z-40">
            <div class="flex items-center justify-between max-w-5xl mx-auto">
                <p class="text-xs text-slate-400 font-medium hidden sm:block">
                    Changes affect <strong>all {{ $meta['label'] }}s</strong>. Per-user overrides always take priority.
                </p>
                <div class="flex items-center gap-3 ml-auto">
                    ...
                    <button type="submit"
                            class="px-8 py-2.5 text-sm font-bold text-white bg-gradient-to-r {{ $meta['gradient'] }} rounded-xl shadow-lg shadow-{{ $meta['color'] }}-600/30 hover:opacity-90 transition inline-flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                        </svg>
                        Save for All {{ $meta['label'] }}s
                    </button>
                </div>
            </div>
        </div>
