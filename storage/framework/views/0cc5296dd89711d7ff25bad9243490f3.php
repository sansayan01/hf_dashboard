    ...
    <div class="max-w-4xl mx-auto pb-12">
        ...
        <div
            ...
            <div class="divide-y divide-slate-100 dark:divide-white/5">
                ...
                <!-- Camp Records Option -->
                <?php if(auth()->user()->hasPermission('finances.view_camp_records')): ?>
                ...
                <?php if(auth()->user()->hasPermission('finances.view_expense')): ?>
                    <a href="<?php echo e(route('expenses.index')); ?>"
                    class="group flex items-center justify-between p-6 sm:px-8 hover:bg-slate-50 dark:hover:bg-white/5 transition-all duration-300">
                    <div class="flex items-center space-x-5">
                        <div
                            class="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-500/10 dark:to-teal-500/10 flex items-center justify-center border border-emerald-100 dark:border-emerald-500/20 shadow-sm text-emerald-500 transform group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300 shrink-0">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div>
                            <h4
                                class="text-lg font-bold text-slate-800 dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                                Expense Tracker</h4>
                            <p class="text-sm text-slate-500 dark:text-slate-400 font-medium mt-0.5 max-w-lg">Record,
                                categorize, and analyze all organizational expenses.</p>
                        </div>
                    </div>
                    <!-- Right Chev -->
                    <div
                        class="text-slate-300 dark:text-slate-600 group-hover:text-emerald-500 dark:group-hover:text-emerald-400 group-hover:translate-x-1 transition-all">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </div>
                ...
                <?php if(auth()->user()->hasPermission('finances.view_income')): ?>
