        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight">
                    <?php echo e(auth()->user()->isSuperAdmin() ? 'Admin Controls' : 'Account Settings'); ?>

                </h1>
                <p class="text-slate-500 dark:text-slate-400 font-medium mt-1">
                    <?php echo e(auth()->user()->isSuperAdmin() ? 'Manage global system permissions and role access levels.' : 'Manage your professional profile and security credentials.'); ?>

                </p>
            </div>
            <?php if(auth()->user()->isSuperAdmin()): ?>
                <div class="flex items-center space-x-2">
                    <span class="px-3 py-1 bg-accent/10 text-accent text-[10px] font-black uppercase tracking-widest rounded-full border border-accent/20"><?php echo e(auth()->user()->getDesignationLabel()); ?></span>
                </div>
            <?php endif; ?>
        </div>
            ...
            <div class="bg-white dark:bg-darkcard rounded-[2rem] shadow-xl border border-slate-100 dark:border-white/5 overflow-hidden">
                <div class="flex flex-col lg:flex-row">
                    ...
                    <div class="lg:w-72 bg-slate-50/50 dark:bg-slate-900/40 border-r border-slate-100 dark:border-white/5 p-6">
                        <nav class="space-y-2" id="permission-tabs">
                            ...
                            <?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <button type="button" onclick="switchTab('<?php echo e($key); ?>')" id="tab-btn-<?php echo e($key); ?>"
                                    ...
                                    <div class="w-8 h-8 rounded-xl bg-white dark:bg-slate-800 shadow-sm flex items-center justify-center text-slate-400 group-hover:text-accent transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?php echo e($data['icon']); ?>" />
                                        </svg>
                                    </div>
                                    <span class="text-slate-500 group-hover:text-slate-800 dark:group-hover:text-white"><?php echo e($data['label']); ?></span>
                                </button>
                    ...
                    <div class="flex-1 p-8 lg:p-12">
                        <!-- Global Form for Permissions -->
                        <form action="<?php echo e(route('profile.permissions')); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div id="permission-content-<?php echo e($key); ?>" class="permission-content hidden animate-in fade-in slide-in-from-right-4 duration-500">
                                    <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                                        <div>
                                            <h2 class="text-2xl font-black text-slate-800 dark:text-white tracking-tight"><?php echo e($data['label']); ?> Permissions</h2>
                                            <p class="text-slate-500 font-medium">Define what users with the <?php echo e($data['label']); ?> role are allowed to perform in the system.</p>
                                        </div>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.dial-toggle','data' => ['id' => 'master_toggle_'.e($key).'','onchange' => 'toggleAllPermissions(\''.e($key).'\', this.checked)']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['id' => 'master_toggle_'.e($key).'','onchange' => 'toggleAllPermissions(\''.e($key).'\', this.checked)']); ?>
                                        ...
                                        <?php $__currentLoopData = $perms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $perm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <label class="group flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-white/5 rounded-3xl cursor-pointer hover:border-accent/30 hover:bg-white dark:hover:bg-slate-800/60 transition-all">
                                                <div class="flex-1 pr-4">
                                                    <p class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest mb-1"><?php echo e(str_replace('_', ' ', $perm->permission_key)); ?></p>
                                                    <p class="text-[10px] text-slate-400 font-bold uppercase">Grant Access</p>
                                                </div>
                                                <?php if (isset($component)) { $__componentOriginalb60c2b1588902dbf70e44e12504bfd46 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginalb60c2b1588902dbf70e44e12504bfd46 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.dial-toggle','data' => ['name' => 'permissions['.e($key).']['.e($perm->permission_key).']','checked' => $perm->is_enabled]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('dial-toggle'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'permissions['.e($key).']['.e($perm->permission_key).']','checked' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($perm->is_enabled)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginalb60c2b1588902dbf70e44e12504bfd46)): ?>
<?php $attributes = $__attributesOriginalb60c2b1588902dbf70e44e12504bfd46; ?>
...
<?php endif; ?>
                                    ...
                                    <?php if($perms->isEmpty()): ?>
                                        <div class="p-8 text-center bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/20 rounded-3xl">
                                            <p class="text-amber-600 dark:text-amber-400 font-bold text-sm">No permissions found for this role. Have you run the database setup?</p>
                                            <a href="<?php echo e(url('/setup-db')); ?>" class="mt-4 inline-block text-xs font-black uppercase tracking-widest text-amber-700 underline">Run Database Setup</a>
                                        </div>
                                    <?php endif; ?>
                            ...
                            <!-- My Account Tab (Visible to Super Admins as 'Admin Profile') -->
                            <div id="permission-content-my-account" class="permission-content hidden animate-in fade-in slide-in-from-right-4 duration-500">
                                <div class="space-y-8">
                                    <!-- User Profile Info Form (Copied from regular user section) -->
                                    <div class="bg-slate-50/50 dark:bg-white/[0.02] p-8 rounded-[2rem] border border-slate-100 dark:border-white/5">
                                        <h3 class="text-xl font-black text-slate-800 dark:text-white mb-6 uppercase tracking-tight">Personal Information</h3>
                                        <form action="<?php echo e(route('profile.update')); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                                            <?php echo csrf_field(); ?>
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div class="space-y-1.5">
                                                    <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pl-1">Full Name (<?php echo e(auth()->user()->getDesignationLabel()); ?>)</label>
                                                    <input type="text" name="full_name" value="<?php echo e(old('full_name', $user->profile->full_name ?? '')); ?>" class="w-full px-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all text-sm font-medium text-slate-700 dark:text-slate-200 outline-none">
                                                </div>
                                                <div class="space-y-1.5">
                                                    <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pl-1">Email Address</label>
                                                    <input type="email" value="<?php echo e($user->email); ?>" disabled class="w-full px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 text-sm font-medium text-slate-400 dark:text-slate-500 cursor-not-allowed">
                                                </div>
                                                <div class="space-y-4 md:col-span-2">
                                                    ...
                                                    <div class="flex flex-col sm:flex-row items-center gap-6 p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-dashed border-slate-300 dark:border-white/10">
                                                        <div class="relative w-20 h-20 flex-shrink-0 group cursor-pointer" onclick="handlePreviewClick('profile_picture_input', 'profile-preview')">
                                                            <?php if($user->profile && $user->profile->profile_picture): ?>
                                                                <img id="profile-preview" src="<?php echo e($user->profile->getProfilePictureUrl()); ?>" class="w-full h-full rounded-2xl object-cover border-2 border-accent/20 shadow-lg transition-all group-hover:scale-105">
                                                            <?php else: ?>
                                                                <div class="initials-placeholder w-full h-full rounded-2xl bg-accent/10 border-2 border-accent/20 flex items-center justify-center text-accent text-xl font-bold transition-all group-hover:scale-105">
                                                                    <?php echo e(substr($user->profile->full_name ?? $user->email, 0, 1)); ?>
                                                        ...
                                                        <div class="flex-1 w-full">
                                                            ...
                                                            <p class="mt-2 text-[10px] text-slate-500 dark:text-slate-400 font-medium">Capture a new picture or upload. JPG/PNG, Max 5MB.</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="pt-4 flex justify-end">
                                                <button type="submit" class="admin-save-btn !min-h-[50px] !px-8">Update Admin Info</button>
                                            </div>
                                        </form>
                                    </div>
                                    ...
                                    <!-- Password Form -->
                                    <div class="bg-slate-50/50 dark:bg-white/[0.02] p-8 rounded-[2rem] border border-slate-100 dark:border-white/5">
                                        <h3 class="text-xl font-black text-slate-800 dark:text-white mb-6 uppercase tracking-tight">Security Credentials</h3>
                                        <form action="<?php echo e(route('profile.password')); ?>" method="POST" class="space-y-6">
                                            ...
                                            <div class="space-y-1.5">
                                                ...
                                                <input type="password" name="current_password" required class="w-full px-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-sm outline-none">
                                            </div>
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div class="space-y-1.5">
                                                    <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pl-1">New Password</label>
                                                    <input type="password" name="password" required class="w-full px-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all text-sm outline-none">
                                                </div>
                                                <div class="space-y-1.5">
                                                    ...
                                                    <input type="password" name="password_confirmation" required class="w-full px-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all text-sm outline-none">
                                                </div>
                                            </div>
                                            <div class="pt-4 flex justify-end">
                                                <button type="submit" class="admin-save-btn !min-h-[50px] !px-8 !background-none !bg-slate-800 !shadow-none">Apply New Password</button>
                                            </div>
                                        </form>
                                    </div>
                        ...
                        <div class="relative z-10 flex flex-col items-center">
                            <div class="relative group/avatar mb-4">
                                <div class="w-24 h-24 rounded-3xl overflow-hidden ring-4 ring-slate-50 dark:ring-white/5 shadow-xl transition-transform group-hover/avatar:scale-105 duration-500">
                                    <?php if($user->profile && $user->profile->profile_picture): ?>
                                        <img src="<?php echo e($user->profile->getProfilePictureUrl()); ?>" class="w-full h-full object-cover">
                                    <?php else: ?>
                            ...
                            <h2 class="text-lg font-black text-slate-800 dark:text-white text-center"><?php echo e($user->profile->full_name ?? 'User'); ?></h2>
                            ...
                            <form action="<?php echo e(route('profile.update')); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-1.5">
                                        ...
                                        <input type="text" name="full_name" value="<?php echo e(old('full_name', $user->profile->full_name ?? '')); ?>" class="w-full px-4 py-3 rounded-xl bg-slate-50/50 dark:bg-white/5 border border-slate-200 dark:border-white/10 focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all text-sm font-medium text-slate-700 dark:text-slate-200 outline-none">
                                    </div>
                                    <div class="space-y-1.5">
                                        <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pl-1">Email Address</label>
                                        <input type="email" value="<?php echo e($user->email); ?>" disabled class="w-full px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-white/10 text-sm font-medium text-slate-400 dark:text-slate-500 cursor-not-allowed">
                                    ...
                                    <div class="space-y-1.5 md:col-span-2">
                                        <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider pl-1">Profile Picture</label>
                                        <div class="mt-1 flex flex-col sm:flex-row items-center gap-6 p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-dashed border-slate-300 dark:border-white/10">
                                            <div class="relative w-20 h-20 flex-shrink-0 group cursor-pointer" onclick="handlePreviewClick('profile_picture_input_regular', 'profile-preview-regular')">
                                                <?php if($user->profile && $user->profile->profile_picture): ?>
                                                    <img id="profile-preview-regular" src="<?php echo e($user->profile->getProfilePictureUrl()); ?>" class="w-full h-full rounded-2xl object-cover border-2 border-accent/20 shadow-lg transition-all group-hover:scale-105">
                                                <?php else: ?>
                                                    <div class="initials-placeholder w-full h-full rounded-2xl bg-accent/10 border-2 border-accent/20 flex items-center justify-center text-accent text-xl font-bold transition-all group-hover:scale-105">
                                                        <?php echo e(substr($user->profile->full_name ?? $user->email, 0, 1)); ?>

                                                    </div>
                                                    <img id="profile-preview-regular" src="#" alt="Preview" class="hidden w-full h-full rounded-2xl object-cover border-2 border-accent/20 shadow-lg transition-all group-hover:scale-105">
                                                <?php endif; ?>
                                                <div class="absolute -bottom-2 -right-2 bg-accent text-white p-1.5 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path>
                                            ...
                                            <div class="flex-1 w-full">
                                                <input type="file" name="profile_picture" id="profile_picture_input_regular" accept="image/*"
                                                    class="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl focus:ring-4 focus:ring-accent/10 transition-all outline-none file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[10px] file:font-black file:uppercase file:tracking-widest file:bg-accent/10 file:text-accent hover:file:bg-accent/20 cursor-pointer"
                                                    onchange="initCropper(this, document.getElementById('profile-preview-regular'))">
                                                <p class="mt-2 text-[10px] text-slate-500 dark:text-slate-400 font-medium">Capture a new picture or upload. JPG/PNG, Max 5MB.</p>
                                            </div>
                                        </div>
                                    </div>
...
<?php $__env->startSection('js'); ?>
    ...
    <script>
        ...
        function toggleAllPermissions(roleKey, isChecked) {
            ...
            const masterToggleId = 'master_toggle_' + roleKey;
            const checkboxes = container.querySelectorAll('.role-permission-checkbox');
            
            checkboxes.forEach(cb => {
                // Skip the master toggle itself to avoid recursion issues
                if (cb.id === masterToggleId) return;

                cb.checked = isChecked;
                
                // Update visual state of dial
                ...
                if (dial) {
                    ...
                    if (isChecked && radioOn) {
                        radioOn.checked = true;
                    } else if (!isChecked && radioOff) {
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\HF\resources\views/profile/edit.blade.php ENDPATH**/ ?>