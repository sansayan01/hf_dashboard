...
<?php $__env->startSection('content'); ?>
    <div class="space-y-8">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative z-[60]">
                    <button type="button" 
                        ...
                        <span class="w-2 h-2 rounded-full <?php echo e(request('view') === 'successful' ? 'bg-emerald-500' : (request('view') === 'not_attended' ? 'bg-rose-500' : 'bg-accent')); ?>"></span>
                        <?php if(request('view') === 'successful'): ?> 
                            View: Successful 
                        <?php elseif(request('view') === 'not_attended'): ?> 
                            View: Not Attended 
                    ...
                    <div id="appointment-filter-dropdown" 
                        ...
                        <div class="grid grid-cols-1 gap-3">
                            <!-- Scheduled Tile -->
                            <a href="<?php echo e(route('appointments.all', ['view' => 'scheduled', 'search' => request('search'), 'date' => request('date')])); ?>" 
                            ...
                            <!-- Successful Tile -->
                            <a href="<?php echo e(route('appointments.all', ['view' => 'successful', 'search' => request('search'), 'date' => request('date')])); ?>" 
                            ...
                            <!-- Not Attended Tile -->
                            <a href="<?php echo e(route('appointments.all', ['view' => 'not_attended', 'search' => request('search'), 'date' => request('date')])); ?>" 
                ...
                <span class="px-4 py-3 bg-slate-100 dark:bg-slate-800 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-xl border border-slate-200/10 dark:border-white/5">
                    <?php echo e($appointments->total()); ?> Records
                </span>
            </div>
        </div>
        ...
        <div class="glass bg-white dark:bg-darkbg/40 p-4 md:p-6 rounded-2xl border border-slate-200/10 dark:border-white/5 shadow-sm">
            <form action="<?php echo e(route('appointments.all')); ?>" method="GET" class="no-loader flex flex-col gap-4">
                <div class="flex flex-col lg:flex-row items-center gap-4 w-full">
                    ...
                    <div class="flex items-center space-x-3 w-full lg:w-auto">
                        ...
                        <?php if(request()->anyFilled(['search', 'date', 'district', 'block', 'gp'])): ?>
                            <a href="<?php echo e(route('appointments.all', ['view' => request('view', 'scheduled')])); ?>" class="px-6 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-danger rounded-2xl transition-all text-[10px] font-black uppercase tracking-widest border border-transparent hover:border-danger/20">
                                Reset
                            </a>
                        <?php endif; ?>
                    </div>
                ...
                <!-- Advanced Filters -->
                <div id="advanced-filters" class="<?php echo e(request()->anyFilled(['district', 'block', 'gp']) ? '' : 'hidden'); ?> grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-slate-100 dark:border-white/5 pt-4">
                    <!-- District -->
                    <div>
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                                    ...
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                                        <td class="p-6">
                                            <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest"><?php echo e($appointment->appointment_id); ?></span>
                                        </td>
                                        <td class="p-6">
                                            <div class="flex items-center space-x-4">
                                                <?php if($appointment->survey): ?>
                                                    <div class="w-10 h-10 bg-accent/10 text-accent dark:text-blue-400 rounded-xl flex items-center justify-center text-sm font-black">
                                                        <?php echo e(substr($appointment->survey->full_name, 0, 1)); ?>

                                                    </div>
                                                    <div>
                                                        <h4 class="font-black text-slate-800 dark:text-white text-sm"><?php echo e($appointment->survey->full_name); ?></h4>
                                                        <p class="text-[10px] text-slate-500 font-bold uppercase tracking-tight mt-0.5"><?php echo e($appointment->survey->patient_id); ?></p>
                                                    </div>
                                                <?php else: ?>
                                        ...
                                        <td class="p-6">
                                            <div class="flex items-center space-x-3">
                                                <div class="flex-shrink-0 w-10 h-10 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 flex flex-col items-center justify-center text-[8px] font-black uppercase">
                                                    ...
                                                    <span class="text-slate-600 dark:text-white text-xs"><?php echo e($appointment->appointment_date->format('d')); ?></span>
                                                </div>
                                                <div>
                                                    <p class="text-xs font-black text-slate-800 dark:text-white"><?php echo e($appointment->appointment_date->format('Y')); ?></p>
                                        ...
                                        <td class="p-6">
                                            <div class="flex items-center space-x-2 text-slate-600 dark:text-slate-300">
                                                ...
                                                <span class="text-xs font-bold"><?php echo e($appointment->location); ?></span>
                                            </div>
                                            <?php if($appointment->status === 'missed_reported'): ?>
                                        ...
                                        <td class="p-6">
                                            <div class="flex items-center space-x-3">
                                                <div class="w-8 h-8 rounded-lg overflow-hidden ring-2 ring-slate-100 dark:ring-slate-800">
                                                    <?php if($appointment->creator && $appointment->creator->profile && $appointment->creator->profile->profile_picture): ?>
                                                        <img src="<?php echo e($appointment->creator->profile->getProfilePictureUrl()); ?>" class="w-full h-full object-cover">
                                                    <?php else: ?>
                                                        <div class="w-full h-full bg-slate-100 dark:bg-slate-800 text-slate-400 flex items-center justify-center text-[10px] font-black">
                                                            <?php echo e(substr($appointment->creator->profile->full_name ?? ($appointment->creator->employee_id ?? 'U'), 0, 1)); ?>

                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                                                <div>
                                                    <?php if(auth()->user()->isSuperAdmin() && $appointment->creator): ?>
                                                        <a href="<?php echo e(route('users.show', $appointment->creator->id)); ?>" class="text-xs font-bold text-accent hover:text-accent/80 transition-colors inline-flex items-center space-x-1 group">
                                                            <span><?php echo e($appointment->creator->profile->full_name ?? ($appointment->creator->employee_id ?? 'Unknown User')); ?></span>
                                                    ...
                                                    <?php else: ?>
                                                        <p class="text-xs font-bold text-slate-700 dark:text-slate-200"><?php echo e($appointment->creator->profile->full_name ?? ($appointment->creator->employee_id ?? 'Unknown User')); ?></p>
                                                    <?php endif; ?>
                                                    <p class="text-[10px] font-medium text-slate-400"><?php echo e($appointment->created_at->format('M d, Y')); ?></p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="p-6 text-right space-x-2 whitespace-nowrap">
                                            <?php if($appointment->status === 'scheduled' && (auth()->user()->isSuperAdmin() || auth()->user()->isOfficeInCharge())): ?>
                                                <form action="<?php echo e(route('appointments.complete', $appointment->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to mark this appointment as successful?')">
                                                    <?php echo csrf_field(); ?>
                                                ...
                                                <form action="<?php echo e(route('appointments.report_missed', $appointment->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Report this appointment as missed?')">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <?php if($appointment->status === 'missed_reported' && (auth()->user()->isSuperAdmin() || auth()->user()->isOfficeInCharge())): ?>
                                                <form action="<?php echo e(route('appointments.complete', $appointment->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Confirm this appointment as SUCCESSFUL?')">
                                                    <?php echo csrf_field(); ?>
                                                ...
                                                <form action="<?php echo e(route('appointments.confirm_missed', $appointment->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Confirm this appointment as NOT ATTENDED?')">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <?php if($appointment->status === 'scheduled' || $appointment->status === 'missed_reported'): ?>
                                                ...
                                                <form action="<?php echo e(route('patients.appointments.destroy', $appointment->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this appointment?')">
                                                    <?php echo csrf_field(); ?>
                                            ...
                                            <?php endif; ?>
                                            <a href="<?php echo e(route('patients.show', $appointment->survey_id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 hover:bg-accent hover:text-white dark:bg-slate-800 dark:hover:bg-accent text-slate-400 transition-all border border-transparent" title="View Patient Profile">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<?php $__env->startSection('js'); ?>
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput && searchForm && appointmentsContainer) {
                searchInput.addEventListener('input', function() {
                    ...
                    debounceTimer = setTimeout(() => {
                        ...
                        if (appointmentsContainer) {
                             appointmentsContainer.classList.add('opacity-50', 'transition-opacity', 'duration-200');
                        }
                        ...
                        .then(html => {
                            ...
                            const newContainer = doc.getElementById('appointments-container');
                            
                            if (newContainer && appointmentsContainer) {
                                appointmentsContainer.innerHTML = newContainer.innerHTML;
                        ...
                        .finally(() => {
                            if (appointmentsContainer) {
                                appointmentsContainer.classList.remove('opacity-50');
                            }
                        });
                    }, 300); // 300ms debounce
                });
