<?php $canBulkApprove = auth()->user()->isSuperAdmin(); ?>
<?php $__empty_1 = true; $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
    <tr class="hover:bg-slate-50 transition-colors group">
        ...
        <td class="px-6 py-4">
            <a href="<?php echo e(route('users.show', $u->id)); ?>" class="flex items-center space-x-3 group">
                <div class="w-10 h-10 rounded-full bg-accent/5 text-accent flex items-center justify-center font-bold overflow-hidden border border-slate-100 dark:border-white/5 group-hover:border-accent/30 transition-colors">
                    ...
                    <?php else: ?>
                        <?php echo e(substr($u->profile?->full_name ?? 'U', 0, 1)); ?>

                    <?php endif; ?>
                </div>
                <div>
                    <p class="text-sm font-bold text-slate-800 group-hover:text-accent transition-colors"><?php echo e($u->profile?->full_name ?? 'Incomplete Profile'); ?></p>
        ...
        <td class="px-6 py-4">
            <p class="text-xs text-slate-500 font-medium"><?php echo e($u->created_at->format('d M, Y')); ?></p>
        </td>
        <td class="px-6 py-4">
            ...
            <div class="flex items-center justify-end space-x-2">
                ...
                <div class="flex items-center justify-end space-x-2 transition-opacity">
                    ...
                    <?php if(auth()->user()->isSuperAdmin()): ?>
                        <a href="<?php echo e(route('users.edit', $u->id)); ?>" class="p-2 text-blue-500 hover:bg-blue-500/10 rounded-lg transition" title="Edit User">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </a>
                        <a href="<?php echo e(route('users.id-card', $u->id)); ?>" target="_blank" class="p-2 text-violet-500 hover:bg-violet-500/10 rounded-lg transition" title="Generate ID Card">
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\HF\resources\views/users/partials/staff_table.blade.php ENDPATH**/ ?>