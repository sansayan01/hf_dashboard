...
<?php $__env->startSection('content'); ?>
    <div class="bg-white dark:bg-darkbg/40 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm overflow-hidden">
        ...
        <div id="filter-panel" class="<?php echo e(request()->anyFilled(['search', 'gender', 'health_issue', 'date_from', 'date_to', 'collector_id']) ? '' : 'hidden'); ?> p-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-darkbg/20 transition-all">
            <form action="<?php echo e(route('surveys.index')); ?>" method="GET" class="no-loader space-y-4">
        ...
        <?php if($surveys->isEmpty()): ?>
            <div class="glass bg-white dark:bg-darkbg/40 rounded-3xl border border-slate-200/10 dark:border-white/5 shadow-xl p-20 text-center">
                ...
                <a href="<?php echo e(route('surveys.create')); ?>" class="inline-block text-accent font-black uppercase tracking-[0.2em] text-[10px] hover:underline">Begin Field Work &rarr;</a>
            </div>
        <?php else: ?>
            <!-- Bulk Actions Bar -->
            <?php if(Auth::user()->isSuperAdmin()): ?>
                <div id="bulk-actions-bar" class="hidden sticky top-0 z-20 bg-accent text-white px-6 py-4 flex items-center justify-between shadow-xl">
                    ...
                    <div class="flex items-center space-x-3">
                        <form id="bulk-delete-form" action="<?php echo e(route('surveys.bulk-destroy')); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete all selected surveys?')">
                            <?php echo csrf_field(); ?>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                            <?php $__currentLoopData = $surveys; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $survey): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                                    <?php if(Auth::user()->isSuperAdmin()): ?>
                                        <td class="p-6 w-10">
                                            <input type="checkbox" class="survey-checkbox w-5 h-5 rounded border-slate-300 text-accent focus:ring-accent accent-accent transition-all cursor-pointer" data-id="<?php echo e($survey->id); ?>">
                                        </td>
                                    <?php endif; ?>
                                    <td class="p-6">
                                        <div class="flex items-center space-x-4">
                                            <div class="w-10 h-10 bg-accent/10 text-accent dark:text-blue-400 rounded-xl flex items-center justify-center text-sm font-black">
                                                <?php echo e(substr($survey->full_name, 0, 1)); ?>

                                            </div>
                                            <div>
                                                <h4 class="font-black text-slate-800 dark:text-white text-sm"><?php echo e($survey->full_name); ?></h4>
                                                <div class="flex items-center space-x-2 text-[10px] text-slate-500 font-bold uppercase tracking-tight mt-0.5">
                                                    <span><?php echo e(ucfirst($survey->gender)); ?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <?php if(auth()->user()->designation !== 'staff'): ?>
                                        <td class="p-6">
                                            ...
                                            <?php elseif(auth()->user()->designation !== 'staff'): ?>
                                                <a href="<?php echo e(route('patients.membership', $survey->id)); ?>" class="inline-flex items-center space-x-2 px-4 py-2 bg-amber-500/10 text-amber-500 rounded-xl hover:bg-amber-500 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest border border-amber-500/10 shadow-sm">
                                    ...
                                    <td class="p-6">
                                        <div class="space-y-1">
                                            <div class="flex items-center space-x-2 text-slate-600 dark:text-slate-300">
                                                ...
                                                <span class="text-xs font-bold"><?php echo e($survey->phone_number); ?></span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-6">
                                        <?php if($survey->health_issues): ?>
                                            <div class="inline-flex items-center space-x-2 px-3 py-1.5 bg-amber-500/10 text-amber-500 rounded-lg border border-amber-500/10">
                                                ...
                                                <span class="text-[10px] font-black uppercase tracking-wider"><?php echo e(Str::limit($survey->health_issues, 20)); ?></span>
                                            </div>
                                        <?php else: ?>
                                    ...
                                    <td class="p-6">
                                        <div class="flex items-center space-x-1">
                                            <a href="<?php echo e(route('patients.appointments.create', $survey->id)); ?>" class="px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:text-indigo-400 dark:hover:bg-indigo-500/20 text-[10px] font-black uppercase tracking-wider transition-colors">
                                                Create
                                            </a>
                                            <a href="<?php echo e(route('patients.appointments.index', $survey->id)); ?>" class="px-3 py-1.5 rounded-lg bg-slate-50 text-slate-500 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 text-[10px] font-black uppercase tracking-wider transition-colors">
                                                View
                                            </a>
                                        </div>
                                    </td>
                                    <td class="p-6">
                                        <div class="flex items-center space-x-3">
                                            <div class="w-8 h-8 rounded-lg overflow-hidden ring-2 ring-slate-100 dark:ring-slate-800">
                                                <?php if($survey->creator->profile && $survey->creator->profile->profile_picture): ?>
                                                    <img src="<?php echo e($survey->creator->profile->getProfilePictureUrl()); ?>" class="w-full h-full object-cover">
                                                <?php else: ?>
                                                    <div class="w-full h-full bg-slate-100 dark:bg-slate-800 text-slate-400 flex items-center justify-center text-[10px] font-black">
                                                        <?php echo e(substr($survey->creator->profile->full_name ?? 'U', 0, 1)); ?>
                                            ...
                                            <div>
                                                <?php if(auth()->user()->isSuperAdmin()): ?>
                                                    <a href="<?php echo e(route('users.show', $survey->creator->id)); ?>" class="text-xs font-bold text-accent hover:text-accent/80 transition-colors inline-flex items-center space-x-1 group">
                                                        <span><?php echo e($survey->creator->profile->full_name ?? 'Unknown'); ?></span>
                                                ...
                                                <?php else: ?>
                                                    <p class="text-xs font-bold text-slate-700 dark:text-slate-200"><?php echo e($survey->creator->profile->full_name ?? 'Unknown'); ?></p>
                                                <?php endif; ?>
                                                <p class="text-[10px] font-medium text-slate-400"><?php echo e($survey->created_at->format('M d, Y')); ?></p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-6 text-right">
                                        <?php if(Auth::id() === $survey->created_by || Auth::user()->canAccess($survey->creator)): ?>
                                            <div class="flex items-center justify-end space-x-2">
                                                <a href="<?php echo e(route('surveys.edit', $survey->id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 hover:bg-accent hover:text-white dark:bg-slate-800 dark:hover:bg-accent text-slate-400 transition-all">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                    </svg>
                                                </a>
                                                <form action="<?php echo e(route('surveys.destroy', $survey->id)); ?>" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this survey?')">
                                                    <?php echo csrf_field(); ?>
